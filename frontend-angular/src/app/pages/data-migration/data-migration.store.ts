...
export type HistoricalEvidenceReport = {
  ...
  groups: { supplierBatch: string; groupKey: string; detectedPageCount: number; approvedPageCount?: number; status: 'suggested' | 'approved' | 'rejected'; confidencePercentage: number; signals: string[]; sourceFileCount: number; decidedBy?: string; decidedAt?: string }[];
  summary: { totalFiles: number; totalPages: number; historicalBillFiles: number; unidentifiedSupplierBatches: string[]; physicalInventoryReady: boolean; supplierOutstandingReady: boolean; groupingApproved: boolean; cutoverApproved: boolean; checksumVerified: boolean; historicalStockEffect: 0; historicalAccountingEffect: 0; historicalGstEffect: 0; liveCrmWrites: 0; complete: boolean };
};
...
export type MigrationGovernance = {
  ...
  openingPayablePreview: {
    ...
    controls: { financeApproved?: boolean; financeApprovedBy?: string; financeApprovedAt?: string; ownerApproved?: boolean; ownerApprovedBy?: string; openingBalanceAccount?: string; payableAccount?: string; supplierAdvanceAccount?: string };
  };
