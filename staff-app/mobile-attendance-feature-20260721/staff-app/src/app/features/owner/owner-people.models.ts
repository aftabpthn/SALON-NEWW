...
export type OwnerAttendancePolicyStatus = "enabled" | "disabled";
export interface OwnerAttendancePolicy {
  branchId?: string; latitude: number | null; longitude: number | null; radiusMeters: number; maxAccuracyMeters: number;
  requireLocation: boolean; requireBiometric: boolean; enforceClockIn: boolean; enforceClockOut: boolean;
  status: OwnerAttendancePolicyStatus; version: number;
}
export type OwnerAttendanceDeviceStatus = "pending" | "trusted" | "revoked";
export interface OwnerAttendanceDevice {
  id: string; branchId: string; staffId: string; staffName: string; deviceLabel: string; platform: string;
  status: OwnerAttendanceDeviceStatus; publicKeyFingerprint: string; registeredAt: string; lastUsedAt: string | null;
  version: number;
}
export type OwnerAttendanceEvidenceDecision = "accepted" | "rejected" | "overridden";
export type OwnerAttendanceAction = "clock_in" | "clock_out" | string;
export interface OwnerAttendanceEvidenceOverride {
  reason: string; overriddenAt: string; overriddenBy?: string; overriddenByName?: string;
}
export interface OwnerAttendanceEvidence {
  id: string; branchId: string; staffId: string; staffName: string; action: OwnerAttendanceAction; capturedAt: string;
  latitude: number | null; longitude: number | null; accuracyMeters: number | null; distanceMeters: number | null; decision: OwnerAttendanceEvidenceDecision;
  reason: string | null; deviceVerified: boolean; biometricVerified: boolean; attendanceId: string | null;
  override?: OwnerAttendanceEvidenceOverride | null; overrideReason?: string | null; overriddenAt?: string | null; version?: number;
}
...
export interface OwnerPayrollItem { id: string; branchId: string; staffId: string; salaryType: string; status: string; version: number; grossAmountPaise: number; overtimeAmountPaise: number; overtimeMinutes: number; overtimeRatePaise: number; overtimeMode: string; overtimeDivisorDays: number; overtimeHoursPerDay: number; overtimeScheduleSource: string; bonusAmountPaise: number; deductionsAmountPaise: number; netAmountPaise: number; pfAmountPaise: number; esicAmountPaise: number; tdsAmountPaise: number; ptAmountPaise: number; salarySource: string; hasStatutoryData: boolean; }
