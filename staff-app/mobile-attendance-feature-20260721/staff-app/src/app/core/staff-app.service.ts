...
import { AttendanceBiometricService } from "./attendance-biometric.service";
...
export type StaffEarnings = {
  ...
  controls: { managedByOwner: boolean; approvalRequired: boolean; hiddenRulesExcluded: boolean };
  performance: { revenuePaise: number; servicesCompleted: number };
  commission: {
    totalPaise: number;
  ...
  incentives: {
    ...
    rules: Array<{
      ...
      currentTier: StaffEarningsTier | null;
      nextTier: StaffEarningsTier | null;
...
export type AttendanceVerificationProgress = "" | "getting-location" | "checking-device" | "verify-biometric" | "submitting";
...
export type StaffPayrollItem = {
  ...
  overtimeMode: "fixed_hourly" | "salary_hourly" | "legacy_unknown";
  overtimeHoursPerDay: number;
  overtimeDivisorDays: number;
  overtimeScheduleSource: string;
  status: string;
  runStatus: string;
  createdAt: string;
};
...
export class StaffAppService {
  ...
  async loadEarnings(query: Record<string, string> = {}): Promise<StaffEarnings> {
    return this.get<StaffEarnings>("/staff-self/earnings", query);
  }
  ...
  async loadQueue(): Promise<StaffQueueData> {
    return this.get<StaffQueueData>("/staff-self/queue");
  }
  ...
  async clockIn(): Promise<MutationResult<StaffAttendance>> {
    if (!this.isOnline()) return this.queueableMutation<StaffAttendance>("POST", "/staff-os/attendance/clock-in", { source: "staff-app" });
    const policy = await this.attendanceVerificationPolicy();
    if (this.isAttendanceEnforced(policy, "clock_in")) return this.verifiedAttendancePunch("clock_in", policy);
  ...
  async clockOut(attendanceId?: string): Promise<MutationResult<StaffAttendance>> {
    if (!this.isOnline()) return this.queueableMutation<StaffAttendance>("POST", "/staff-os/attendance/clock-out", { attendanceId });
    const policy = await this.attendanceVerificationPolicy();
    if (this.isAttendanceEnforced(policy, "clock_out")) return this.verifiedAttendancePunch("clock_out", policy, attendanceId);
    return this.queueableMutation<StaffAttendance>("POST", "/staff-os/attendance/clock-out", { attendanceId });
  }

  async attendanceVerificationPolicy(): Promise<AttendanceVerificationPolicy> {
    return this.get<AttendanceVerificationPolicy>("/staff-self/attendance-verification-policy");
  }

  private isAttendanceEnforced(policy: AttendanceVerificationPolicy, action: "clock_in" | "clock_out"): boolean {
    const enabled = policy.enforcementRequired === true || policy.enabled === true;
    const actionSetting = action === "clock_in" ? policy.enforceClockIn : policy.enforceClockOut;
    return enabled && actionSetting !== false;
  }

  private async verifiedAttendancePunch(action: "clock_in" | "clock_out", policy: AttendanceVerificationPolicy, attendanceId?: string): Promise<MutationResult<StaffAttendance>> {
    this.attendanceVerificationEvidence.set(null);
    if (!this.isOnline()) throw new Error("Verified attendance requires an internet connection and cannot be queued.");
    if (!this.attendanceBiometric.isSupportedPlatform()) throw new Error("Verified attendance is supported only in the Android staff app.");
    try {
      this.attendanceVerificationProgress.set("getting-location");
      const location = await this.attendanceBiometric.location(policy.maxAccuracyMeters);
      this.attendanceVerificationEvidence.set({ accuracyMeters: location.accuracyMeters, radiusMeters: policy.radiusMeters });

      this.attendanceVerificationProgress.set("checking-device");
      const deviceId = this.attendanceBiometric.deviceId();
      let device = await this.findAttendanceDevice(deviceId);
      if (!device) {
        const key = await this.attendanceBiometric.publicKey();
        if (key.hardwareBacked !== true) throw new Error("Secure hardware-backed biometric keys are not supported on this device.");
        device = await this.post<AttendanceDevice>("/staff-self/attendance-device/register", {
          deviceId,
          deviceLabel: this.attendanceBiometric.deviceLabel(),
          platform: "android",
      ...
      const challenge = await this.post<{ challengeId: string; nonceBase64: string; expiresAt: string }>("/staff-self/attendance-challenge", {
        action, attendanceId, deviceId, ...location
      });
      this.attendanceVerificationProgress.set("verify-biometric");
      const signed = await this.attendanceBiometric.signNonce(challenge.nonceBase64);
      this.attendanceVerificationProgress.set("submitting");
      const response = await this.post<StaffAttendance | MutationResult<StaffAttendance> | { attendance: StaffAttendance; evidence?: AttendanceVerificationEvidence }>("/staff-self/attendance-verified-punch", {
        challengeId: challenge.challengeId,
        deviceId,
        signatureBase64: signed.signatureBase64
      });
      const evidence = this.extractAttendanceEvidence(response);
      if (evidence) this.attendanceVerificationEvidence.set(evidence);
      if (this.isMutationResult(response)) return response;
      return { state: "completed", data: "attendance" in response ? response.attendance : response };
    } catch (error) {
      const message = this.errorMessage(error, "Unable to verify attendance.");
      this.error.set(this.attendanceErrorMessage(message));
      throw error;
    } finally {
  ...
  private async findAttendanceDevice(deviceId: string): Promise<AttendanceDevice | null> {
    try { return await this.get<AttendanceDevice>("/staff-self/attendance-device", { deviceId }); }
    catch (error) {
  ...
  private assertTrustedAttendanceDevice(device: AttendanceDevice): void {
    ...
    if (device.denied || ["denied", "rejected", "revoked", "blocked"].includes(status)) throw new Error("This attendance device was denied. Ask the owner to approve another device.");
    if (!(device.trusted === true || device.approved === true || ["trusted", "approved", "active"].includes(status))) {
      throw new Error("This device is awaiting owner approval. Attendance was not recorded.");
    }
  }

  private isMutationResult(value: unknown): value is MutationResult<StaffAttendance> {
    return !!value && typeof value === "object" && ["completed", "queued"].includes(String((value as { state?: unknown }).state));
  }
  private extractAttendanceEvidence(value: unknown): AttendanceVerificationEvidence | null {
    if (!value || typeof value !== "object") return null;
    const record = value as Record<string, unknown>;
    const evidence = record["evidence"] || record["verificationEvidence"] || (record["data"] && typeof record["data"] === "object" ? (record["data"] as Record<string, unknown>)["verificationEvidence"] : null);
    if (evidence && typeof evidence === "object") return evidence as AttendanceVerificationEvidence;
    if (["distanceMeters", "accuracyMeters", "radiusMeters"].some((key) => typeof record[key] === "number")) {
      return { distanceMeters: record["distanceMeters"] as number | undefined, accuracyMeters: record["accuracyMeters"] as number | undefined, radiusMeters: record["radiusMeters"] as number | undefined };
    }
    return null;
  }
  private attendanceErrorMessage(message: string): string {
    if (/outside|radius|too far/i.test(message)) return `You are outside the allowed attendance area. ${message}`;
    if (/accuracy/i.test(message)) return message;
    return message;
  }
