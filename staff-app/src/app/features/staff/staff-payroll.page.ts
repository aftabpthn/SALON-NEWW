...
export class StaffPayrollPage implements OnInit { readonly payroll = signal<StaffPayrollItem[]>([]); readonly loading = signal(false); constructor(readonly staff: StaffAppService) {} ngOnInit() { if (this.canSeePayroll()) void this.load(); } async load() { const cached = this.staff.readStoredData<StaffPayrollItem[]>("payroll"); if (cached) { this.payroll.set(cached); this.loading.set(false); } else { this.loading.set(true); } try { const data = await this.staff.payroll(); this.payroll.set(data); this.staff.writeStoredData("payroll", data); } finally { this.loading.set(false); } } canSeePayroll(): boolean { return this.staff.hasAnyPermission(["read:payroll", "read:finance"]); } payrollAmount(item: StaffPayrollItem): number { return Number(item.netAmountPaise ?? item.grossAmountPaise ?? 0); } }
