  ...
  template: `
    <section class="page-stack">
      ...
      <div class="metrics-grid" *ngIf="summary()?.metrics as metrics">
        <aura-kpi-card tone="teal" target="/kpi-details/customer-360/clients"><span>Clients</span><strong>{{ metrics.clients }}</strong></aura-kpi-card>
        <aura-kpi-card tone="blue" target="/kpi-details/customer-360/total-ltv"><span>Total LTV</span><strong>{{ metrics.totalLtv | currency: 'INR':'symbol':'1.0-0' }}</strong></aura-kpi-card>
        <aura-kpi-card tone="green" target="/kpi-details/customer-360/avg-spend"><span>Avg spend</span><strong>{{ metrics.avgSpend | currency: 'INR':'symbol':'1.0-0' }}</strong></aura-kpi-card>
        <aura-kpi-card tone="red" target="/kpi-details/customer-360/high-risk"><span>High risk</span><strong>{{ metrics.highRisk }}</strong></aura-kpi-card>
      </div>
