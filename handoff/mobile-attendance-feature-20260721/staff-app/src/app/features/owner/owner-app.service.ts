...
import { OwnerAttendance, OwnerAttendanceDevice, OwnerAttendanceDeviceStatus, OwnerAttendanceEvidence, OwnerAttendanceEvidenceDecision, OwnerAttendancePolicy, OwnerLeave, OwnerLeaveDetail, OwnerListResponse, OwnerPayroll, OwnerPayrollDetail, OwnerStaff, OwnerStaffDetail, OwnerStaffWrite } from "./owner-people.models";
...
export class OwnerAppService {
  ...
  ownerAttendancePolicy(branchId: string): Promise<OwnerAttendancePolicy> { return this.get(`/owner-console/people/attendance-policy/${encodeURIComponent(branchId)}`); }
  saveOwnerAttendancePolicy(branchId: string, policy: OwnerAttendancePolicy): Promise<OwnerAttendancePolicy> { return this.put(`/owner-console/people/attendance-policy/${encodeURIComponent(branchId)}`, policy); }
  ownerAttendanceDevices(params: { branchId: string; staffId?: string; status?: OwnerAttendanceDeviceStatus | "" }): Promise<OwnerAttendanceDevice[]> {
    return this.get<{ items: OwnerAttendanceDevice[] }>("/owner-console/people/attendance-devices", { branchId: params.branchId, ...(params.staffId ? { staffId: params.staffId } : {}), ...(params.status ? { status: params.status } : {}) }).then((response) => response.items);
  }
  setOwnerAttendanceDeviceStatus(id: string, status: "trusted" | "revoked", version: number): Promise<OwnerAttendanceDevice> { return this.patch(`/owner-console/people/attendance-devices/${encodeURIComponent(id)}/status`, { status, version }); }
  ownerAttendanceEvidence(params: { branchId: string; staffId?: string; from?: string; to?: string; decision?: OwnerAttendanceEvidenceDecision | "" }): Promise<OwnerAttendanceEvidence[]> {
    return this.get<{ items: OwnerAttendanceEvidence[] }>("/owner-console/people/attendance-evidence", { branchId: params.branchId, ...(params.staffId ? { staffId: params.staffId } : {}), ...(params.from ? { from: params.from } : {}), ...(params.to ? { to: params.to } : {}), ...(params.decision ? { decision: params.decision } : {}) }).then((response) => response.items);
  }
  overrideOwnerAttendanceEvidence(id: string, reason: string): Promise<OwnerAttendanceEvidence> { return this.post<{ attendance: OwnerRecord; evidence: OwnerAttendanceEvidence }>(`/owner-console/people/attendance-evidence/${encodeURIComponent(id)}/override`, { reason }).then((response) => response.evidence); }
