  ...
  template: `
    <section class="page dashboard-page" [attr.aria-busy]="initialLoading()">
      @if (blockingError()) {
        ...
        @if (verificationStatus()) { <section staffPageState class="notice" role="status" aria-live="polite">{{ verificationStatus() }}</section> }
        @if (verificationEvidence()) { <section staffPageState class="notice success" role="status">{{ verificationEvidence() }}</section> }
        @if (refreshWarning()) {
...
export class StaffDashboardPage implements OnInit, OnDestroy {
  ...
  readonly verificationStatus = computed(() => this.verificationProgressLabel(this.staff.attendanceVerificationProgress()));
  readonly verificationEvidence = computed(() => this.verificationEvidenceLabel());
  ...
  private verificationProgressLabel(progress: ReturnType<StaffAppService["attendanceVerificationProgress"]>): string {
    return ({ "getting-location": "Getting precise location…", "checking-device": "Checking attendance device…", "verify-biometric": "Verify your device biometric…", submitting: "Submitting verified attendance…", "": "" } as const)[progress];
  }
  private verificationEvidenceLabel(): string {
    const evidence = this.staff.attendanceVerificationEvidence();
    if (!evidence) return "";
    const parts = [];
    if (typeof evidence.distanceMeters === "number") parts.push(`Distance ${Math.round(evidence.distanceMeters)} m`);
    if (typeof evidence.accuracyMeters === "number") parts.push(`accuracy ${Math.round(evidence.accuracyMeters)} m`);
