...
import { overtimeScheduleByStaff, resolveOvertimePay } from "./staff-overtime-pay.service.js";
import { assertAttendanceVerification } from "./attendance-verification-policy.service.js";
...
export class StaffOsService {
  ...
  clockIn(payload = {}, access) {
    assertAttendanceVerification(payload, access, "clock_in");
    return this.createAttendanceEvent("clock_in", payload, access);
  }

  clockOut(payload = {}, access) {
    assertAttendanceVerification(payload, access, "clock_out");
  ...
  generatePayroll(payload = {}, access) {
    ...
    const overtimeSchedule = overtimeScheduleByStaff({ tenantId: access.tenantId, branchId, periodStart, periodEnd });
    ...
    const trx = db.transaction(() => {
      ...
      for (const staff of staffRows) {
        ...
        const salaryRow = db.prepare(`SELECT new_ctc FROM salary_revision_history
          WHERE tenant_id = @tenantId AND staff_id = @staffId AND approval_status = 'approved' AND effective_date <= @periodEnd
          ORDER BY effective_date DESC, approved_at DESC LIMIT 1`).get({ tenantId: access.tenantId, staffId: staff.id, periodEnd });
        ...
        const overtimePay = resolveOvertimePay({
          salaryProfile,
          salaryAmountRupees: effectiveGross,
          overtimeMinutes: periodOvertimeMinutes,
          periodEnd,
          schedule: overtimeSchedule.get(String(staff.id)),
          fallbackRateRupees: submittedOtHours > 0 ? submittedOtAmount / submittedOtHours : 0
        });
        const overtimeAmount = overtimePay.amountPaise / 100;
        ...
        const payrollItem = {
          ...
          staff_id: staff.id,
          gross_amount: Math.round(itemGross * 100) / 100,
          overtime_amount: overtimeAmount,
          bonus_amount: Math.round(bonusAccrual * 100) / 100,
          ...
          statutory_json: json({
            ...
            overtimeMinutes: periodOvertimeMinutes,
            overtimeAmount,
            overtimeAmountPaise: overtimePay.amountPaise,
            overtimeRatePaise: overtimePay.hourlyRatePaise,
            overtimeMode: overtimePay.mode,
            overtimeSalaryBasisPaise: overtimePay.salaryBasisPaise,
            overtimeDivisorDays: overtimePay.divisorDays,
            overtimeHoursPerDay: overtimePay.hoursPerDay,
            overtimeScheduleSource: overtimePay.scheduleSource,
            bonusAmount,
            bonusAccrual,
        ...
        db.prepare(`INSERT INTO staff_payroll_items (id, tenant_id, payroll_run_id, branch_id, staff_id, gross_amount, overtime_amount, bonus_amount, deduction_amount, net_amount, statutory_json)
          VALUES (@id, @tenant_id, @payroll_run_id, @branch_id, @staff_id, @gross_amount, @overtime_amount, @bonus_amount, @deduction_amount, @net_amount, @statutory_json)`).run(payrollItem);
      }
      db.prepare(`UPDATE staff_payroll_runs SET gross_amount = @grossAmount, deductions_amount = @deductionsAmount, net_amount = @netAmount, updated_at = @updatedAt WHERE id = @id AND tenant_id = @tenantId`)
        .run({ grossAmount: Math.round(gross * 100) / 100, deductionsAmount: Math.round(deductions * 100) / 100, netAmount: Math.round(net * 100) / 100, updatedAt: now(), id: run.id, tenantId: access.tenantId });
  ...
  mobilePayroll(query = {}, access) {
    ...
    return db.prepare(`SELECT i.*, r.period_start, r.period_end, r.status AS run_status
      FROM staff_payroll_items i
      JOIN staff_payroll_runs r ON r.id = i.payroll_run_id AND r.tenant_id = i.tenant_id
      WHERE i.tenant_id = @tenantId AND i.staff_id = @staffId AND r.status IN ('approved', 'paid', 'processed')
      ORDER BY i.created_at DESC LIMIT 12`).all({ tenantId: access.tenantId, staffId }).map((raw) => {
        const row = rowToCamel(raw);
        const statutory = parseJsonObject(row.statutoryJson);
        const hasOvertimeMetadata = Boolean(statutory.overtimeMode || statutory.overtimeRatePaise || statutory.overtimeAmountPaise);
        return {
          ...row,
          periodStart: row.periodStart || "",
          periodEnd: row.periodEnd || "",
          status: row.status || "pending",
          runStatus: row.runStatus || "approved",
          grossPay: Math.round(Number(row.grossAmount || 0) * 100),
          netPay: Math.round(Number(row.netAmount || 0) * 100),
          overtimeMinutes: Number(statutory.overtimeMinutes || 0),
          overtimeAmountPaise: Number(statutory.overtimeAmountPaise ?? Math.round(Number(row.overtimeAmount || statutory.overtimeAmount || 0) * 100)),
          overtimeRatePaise: Number(statutory.overtimeRatePaise || 0),
          overtimeMode: hasOvertimeMetadata ? (statutory.overtimeMode || "fixed_hourly") : "legacy_unknown",
          overtimeHoursPerDay: Number(statutory.overtimeHoursPerDay || 0),
          overtimeDivisorDays: Number(statutory.overtimeDivisorDays || 0),
          overtimeScheduleSource: statutory.overtimeScheduleSource || ""
        };
      });
  }
