...
| F10 | AI Scribe recording with consent | Staff App records granted/refused consent before capture, streams bounded audio to the configured AI service, never persists audio and supports consent revocation with transcript destruction | SOURCE READY; PROVIDER/REAL-DEVICE UAT PENDING | 13 |
| F11 | Transcription and structured clinical summary | Mounted `/staff-scribe` routes call the governed transcription service and persist transcript, structured summary, provider/model evidence and immutable events | SOURCE READY; PROVIDER/ACCURACY UAT PENDING | 13 |
| F12 | AI-assisted form fill with staff review/edit | Provider suggestions remain drafts; the Staff App requires staff review and the Rust service writes only submitted responses through the existing versioned form workflow | SOURCE READY; AUTHENTICATED CLINICAL UAT PENDING | 13 |
| F13 | Permission/license/model/audit governance for Scribe | Separate read/manage permissions, consent evidence, tenant/branch/appointment scope, provider failures, immutable audit events and retention/revocation controls are wired | SOURCE READY; PRIVACY/PROVIDER CERTIFICATION PENDING | 13 |
