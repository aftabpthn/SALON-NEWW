...
- `GET /inventory/ledger` is the immutable transaction history for sales, refunds, purchases, purchase returns, transfers/reversals, consumption, adjustments and kit bundle/unbundle movements. Every row carries saved movement cost, source identity, actor/appointment/service/staff/client/container attribution where applicable, batch allocations and snapshot trust status. `limit` is capped at 2,000 and `offset` provides stable `created_at DESC, id DESC` pagination so reports can read the complete filtered history without truncation.
...
## Phase 13 procurement completion

- `inventory_policies.stock_action_matrix` controls receipt, return, transfer, adjustment, audit, consumption and kit mutations. The shared inventory idempotency middleware reads the saved branch policy before reserving a mutation and rejects disabled actions without invoking a handler.
- `inventory_policies.purchase_order_settings` owns the PO prefix, approval requirement/threshold, bulk-raise switch and opt-in supplier electronic delivery. `POST /purchases/orders/bulk-raise` locks and raises 1–100 draft orders in one transaction and appends one event per order.
- When electronic delivery is enabled, `POST /purchases/orders/{id}/send` queues the supplier email inside the same transaction as the sent event. Missing supplier email fails the transaction; the existing outbox owns idempotency, retry and provider evidence.
- `PATCH /inventory/bulk` atomically changes only center availability, webstore availability and reorder point for 1–200 scoped products. It cannot write stock or cost, and no row changes when any requested ID is absent.
- `POST /inventory/{id}/clone` reuses normal product validation and creates zero opening stock, no copied barcode and no copied ledger. A cost-masked actor cannot clone hidden purchase cost.
- `POST /inventory/{id}/discontinue` requires a reason, zero on-hand stock, zero batch/container balance and no open PO or in-transit transfer; an optional replacement must be active in the same branch. `POST /inventory/{id}/reactivate` restores the master with a reason. Legacy `DELETE /inventory/{id}` is backward-compatible safe deactivation, never hard deletion.
- `inventory_product_lifecycle_events` is append-only and appears in Product 360. Historical stock, batch, purchase, sale, valuation and GL rows remain unchanged.
- `inventory_policies.label_settings` owns MRP/price caption, visible label fields, dimensions and columns. Product multi-select and GRN label printers share it and retain Code39 validation.
- Inventory Reports now includes full paginated Current Stock/Product Catalog, ledger-backed permission-gated COGS, Ageing, Near Expiry, Batch Traceability, Stock Variance and Supplier Performance. Cost fields and COGS are not rendered or exported for cost-masked roles.
