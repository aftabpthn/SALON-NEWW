...
- Frontend: Angular standalone app under `src/app`.
- Backend: Express JavaScript ESM under `server`.
- Database: SQLite through `better-sqlite3`.
- Auth: JWT access tokens, refresh tokens, RBAC, tenant and branch headers.
- Realtime: WebSocket.
- DevOps: npm scripts, GitHub repository workflow, SQLite backups.

### 8. Repository Structure

- `src/`: Angular application, routes, pages, core services, shared UI.
- `server/`: Express app, routes, services, repositories, middleware,
  validators, migrations, and jobs.
- `docs/`: domain guides, audit reports, roadmap, runbooks, and operational
  references.
- `tests/`: focused backend and wiring checks.
- `scripts/`: maintenance, backup, and verification helpers.
...
2. Run `npm install` only when dependencies are missing or changed.
3. Copy `.env.example` to `.env` and set production secrets.
4. Start backend with `npm run api`.
5. Start frontend with `npm run client`.
6. Verify backend at `http://127.0.0.1:4000/health` and frontend at
   `http://127.0.0.1:4300`.
...
Important contracts include `JWT_SECRET`, `JWT_ACCESS_TTL_SECONDS`,
`JWT_REFRESH_TTL_DAYS`, `DATABASE_PATH`, `AI_PROVIDER`, `OPENAI_API_KEY`,
...
Angular pages call `ApiService`; Express routes validate and authorize; services
hold business logic; repositories own SQL; SQLite stores tenant-scoped truth;
...
Follow existing Angular, Express, SQLite, repository, service, validator, and
...
SQLite lock errors, auth refresh failures, and missing environment variables
before changing code.

### 21. FAQ

- Is this Angular + Express + SQLite? Yes.
- Is Mongo/Postgres/Redis planned? No migration is proposed in normal work.
