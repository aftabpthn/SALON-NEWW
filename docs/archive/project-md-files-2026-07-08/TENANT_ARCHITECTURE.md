...
- Shared SQLite database, shared schema, **row-level isolation**: every tenant-owned table carries `tenantId` (+ `branchId`), and every repository read/write filters on it.
