...
1. **SQLite database** `data/salon-crm.sqlite` — via online backup (`npm run backup:db`, `scripts/backup-database.mjs`), WAL-safe, non-blocking.
...
- **Monthly drill:** restore latest backup to a scratch environment → `PRAGMA integrity_check` → `npm run check:server` → sample tenant data spot-check → drill logged.
...
- Continuous WAL shipping to shrink RPO toward minutes.
