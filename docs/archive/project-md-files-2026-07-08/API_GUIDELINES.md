...
- Route files in `server/routes`, registered with a single line in `server/app.js` (`operations.routes.js` is protected — wrap, don’t edit).
- Validation at the boundary via `server/validators`; unknown fields rejected or ignored explicitly.
...
- `api.test.js` and feature wiring tests pass.
