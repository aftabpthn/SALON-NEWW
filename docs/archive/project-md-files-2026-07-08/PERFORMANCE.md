...
1. **Short transactions.** Prepare all data first; the better-sqlite3 transaction only writes. Nothing slow (network, ML, rendering) inside a transaction.
...
6. **Workers for long work.** Broadcasts, exports, imports, AI calls, snapshot builds run in `server/jobs`/`server/workers`, not requests.
...
- Watch SQLite file size and query timings as tenants grow; index review at 10x data growth.
