...
- **Schema:** only via new sequential file in `server/migrations`; additive-first.
...
- Backend: Express ESM only, route -> validator/middleware -> service ->
  repository.
- Database: SQLite via `better-sqlite3`, named params, tenant/branch scoped
...
- `server/routes`: HTTP contracts and route composition.
- `server/services`: business workflows and domain orchestration.
- `server/repositories`: SQL and persistence only.
- `server/migrations`: additive schema evolution.
