...
One consistent way to raise, translate, log and present errors across the Express
backend, workers and the Angular SPA.

## 2. Error Taxonomy

| Class | HTTP | Behaviour |
| --- | --- | --- |
| Validation error (bad payload shape) | 400 | Rejected at the boundary by `server/validators`; field detail in `error.details` |
...
2. **Fail the transaction, not half of it.** Any error inside a better-sqlite3 transaction aborts the whole write; partial state is impossible by construction.
