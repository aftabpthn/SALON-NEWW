...
Define the day/week/month calendar experience: staff columns, chair assignment, drag-and-drop status changes, walk-ins and realtime updates.

## 2. Scope

- Day / week / month calendar views
- Staff and chair (resource) assignment
- Drag status changes and reschedule
- Walk-in creation from the calendar
...
- Render conflict-free slots for every staff member in IST business time
- Reflect booking changes to all connected front-desk sessions in realtime
- Respect branch scoping — staff see only their assigned branch calendar
...
1. Open calendar → load appointments for branch + date range in one query
2. Drag appointment → optimistic UI update → PATCH API → WebSocket broadcast
3. Walk-in → quick-create modal → slot + staff auto-suggested
...
- No double-booking of a staff member or chair for overlapping times
- Calendar loads a full day view in under 500 ms for 200 appointments
- All times stored in UTC/ISO, displayed in IST
