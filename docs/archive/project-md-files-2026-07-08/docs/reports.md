...
> **Primary AI Role:** Reporting Architect
...
Define the reports command center: financial, sales, settlement, staff, client and inventory reports, their data contracts and export formats.

## 2. Scope

- Daily revenue, daily sheet financial control and financial summary reports
- Invoice settlement, unpaid due recovery and deleted-invoice approval reports
- Product in/out, product sales and discount intelligence reports
- Excel (xlsx) export and print layouts

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Every report reads from payment-truth and journal sources, never ad-hoc counters
- All reports are tenant + branch scoped with role-gated access
- Report totals cross-foot: detail rows always sum to the header total
...
1. Report requested → parameters validated → single scoped SQL read (named params)
2. Rows shaped by a dedicated service (no SQL in routes)
3. Export via xlsx with the same numbers as the on-screen view
...
- On-screen, printed and exported totals are identical
- Reports tests (*-report.test.js) pass
- A report never mutates data
