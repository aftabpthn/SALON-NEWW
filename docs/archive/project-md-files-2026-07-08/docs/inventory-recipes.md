...
Define service recipes: which consumables (and how much) each service uses, driving batch-aware auto deduction and true service costing.

## 2. Scope

- Recipe definitions per service (product + quantity per use)
- Batch-aware auto deduction on service completion
- True service cost for profit intelligence
- Usage variance and waste analysis

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Recipe changes are versioned; historical costing uses the recipe active at service time
- Deduction uses WMA cost consistent with accounting
- Missing stock at deduction time raises an alert instead of silently skipping
...
1. Service defined → recipe attached with consumable quantities
2. Invoice completed → recipe expanded → stock deducted per branch
3. Profit intelligence consumes recipe cost per service line
...
- Every deduction row references invoice line + recipe version
- Recipe cost feeds docs/profit-intelligence.md calculations
- Zero-recipe services are reported, not errored
