...
Define loyalty points: earn rules, redemption at POS, expiry, tier logic and liability accounting.

## 2. Scope

- Earn rules (per visit / per rupee spent / per service)
- Redemption at POS with value mapping
- Point expiry and tier levels
- Loyalty liability in accounting

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Points balance derives from earn/redeem ledger rows
- Redemption value is capped and permission-controlled
- Outstanding points are a measurable liability
...
1. Invoice completed → earn rule evaluates → points ledger credit
2. Redemption at POS → points debited → discount applied as loyalty tender
3. Expiry job debits lapsed points with audit trail
...
- Points can never be redeemed beyond balance
- Earn and redeem rows always reference the source invoice
- Tier recalculation is idempotent
