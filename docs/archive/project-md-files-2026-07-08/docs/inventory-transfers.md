...
Define inter-branch stock transfers: request, approval, dispatch, receipt, in-transit tracking and valuation.

## 2. Scope

- Transfer request and approval workflow
- Dispatch and receive with quantity confirmation
- In-transit stock state
- Transfer valuation at WMA cost

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Stock is decremented at dispatch and incremented only at confirmed receipt
- In-transit quantity is visible and reconciled
- Discrepancies (short receipt) create a variance record requiring approval
...
1. Branch A requests → manager approves → dispatch decrements source with batch detail
2. Goods travel (in-transit state) → Branch B receives and confirms quantities
3. Variance, if any, routed for approval and adjustment posting
...
- Source and destination movements always reference the same transferId
- Transfers respect tenantId — cross-tenant transfer is impossible
- Valuation moves at cost with no P&L effect
