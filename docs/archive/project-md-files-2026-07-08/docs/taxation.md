...
Define GST handling: rate mapping, CGST/SGST/IGST split, inclusive/exclusive pricing, invoices, HSN/SAC codes and statutory reporting.

## 2. Scope

- GST rate master and HSN/SAC mapping for services and products
- CGST/SGST vs IGST determination
- Tax-inclusive and tax-exclusive price handling
- GST summary reporting for filing support

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Tax computed once at billing in integer paise, stored per invoice line
- Rounding follows statutory rules and is applied at the invoice level
- Tax rate changes are effective-dated; old invoices keep their original tax
...
1. Item billed → rate from master by HSN/service mapping → line tax computed
2. Invoice totals aggregate line taxes → GST breakup printed on invoice
3. Periodic GST summary report generated per tenant for filing
...
- statutory-compliance.test.js passes
- Invoice GST breakup always sums exactly to invoice tax total
- No tax amount is ever recomputed retroactively on stored invoices
