...
Define gift card issuance, activation, redemption, balance tracking, expiry and liability accounting.

## 2. Scope

- Card issuance (physical code / digital) and activation
- Full and partial redemption at POS
- Balance inquiry and transfer rules
- Expiry policy and breakage handling

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Card balance derives from an issuance/redemption ledger
- Card codes are unique per tenant and unguessable
- Issued value sits as deferred liability until redeemed or expired
...
1. Card sold → payment captured → liability posted → code activated
2. Redemption at POS → balance checked → tendered as gift-card payment
3. Expiry job recognises breakage per policy with journal entries
...
- A card can never be redeemed past its balance or before activation
- Redemptions are tenant-scoped — cards never work across tenants
- Every card event is audit-logged
