...
> **Primary AI Role:** Salon Domain Expert
...
Define the appointment lifecycle: creation, confirmation, deposits, multi-service scheduling, waitlists, no-show handling and QR check-in.

## 2. Scope

- Appointment lifecycle states and transitions
- Smart slot recommendation and auto staff assignment
- Deposits and advance payments
- Waitlist and online booking requests
- Conflict prevention and queue prediction

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Enforce a single valid state machine (requested → confirmed → checked-in → in-service → completed / cancelled / no-show)
- Prevent conflicting bookings for staff, chair and client
- Link deposits to invoices so advances settle correctly at billing time
...
1. Client books (front desk / online widget / WhatsApp AI) → slot validated → appointment created
2. Reminders queued via workflow engine → confirmation tracked
3. Completion → POS billing prefilled with services, staff and advance adjustment
...
- State transitions outside the state machine are rejected with a typed error
- Protected services smart-booking.service.js and booking-portal.service.js are wrapped, never modified
- Every appointment row carries tenantId + branchId and an activity trail
