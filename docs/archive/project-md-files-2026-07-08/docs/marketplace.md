...
Define the app marketplace: listings, connectors, plugin model, install/uninstall lifecycle and tenant-level enablement.

## 2. Scope

- Marketplace listings and connector catalog
- Plugin registration and lifecycle
- Tenant-level install, configure, disable
- Connector health and version compatibility

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Plugins run behind feature toggles and can be disabled without deploys
- Connector credentials are tenant-scoped and encrypted
- A misbehaving plugin cannot break core POS/booking flows
...
1. Listing published → tenant installs → configuration validated → enabled
2. Connector events flow through the standard integration adapters
3. Uninstall disables hooks and retains data per retention policy
...
- Core flows pass tests with all plugins disabled
- Install/uninstall events are audit-logged
- Version incompatibilities block enablement with a clear message
