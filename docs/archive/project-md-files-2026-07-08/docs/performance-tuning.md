...
> **Primary AI Role:** Performance Engineer
...
Practical tuning guide: SQLite pragmas and indexing, query patterns, Angular bundle and change detection, WebSocket fan-out and worker throughput.

## 2. Scope

- SQLite: WAL mode, indexes, prepared statements, transaction batching
- API: N+1 elimination, pagination, response shaping
- Angular: lazy routes, OnPush, bundle budgets
- Workers and WebSocket broadcast efficiency

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Hot paths (POS billing, calendar load, client search) have documented budgets and measurements
- Indexes exist for every tenantId+branchId+frequent-filter combination
- Performance regressions are caught by the checks listed here before merge
...
1. Measure first (timings in dev) → identify hotspot → tune → re-measure
2. New list endpoints ship paginated by default
3. Budgets reviewed when data volumes grow 10x
...
- POS invoice save under 300 ms at p95 on reference hardware
- Calendar day view under 500 ms for 200 appointments
- Root PERFORMANCE.md budgets are all measured, not aspirational
