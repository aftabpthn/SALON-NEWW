...
Define backup policy for the SQLite database and file assets: schedule, retention, encryption, verification and offsite copies.

## 2. Scope

- SQLite snapshot backups via scripts/backup-database.mjs and npm run backup:db
- File/asset backups
- Retention schedule and encryption
...
- Backups run on schedule without blocking the live database (online backup API)
- Every backup is checksummed and test-restored on a cadence
- Retention: dailies, weeklies and monthlies per policy table in this doc
...
1. Scheduled job → online SQLite backup → compress + checksum → encrypted offsite copy
2. Verification job restores latest backup to scratch and runs integrity checks
3. Alerts fire on missed or failed backups
...
- A backup exists no older than 24h at all times
- Restore drill succeeds monthly and is logged
- Backups never contain plaintext secrets
