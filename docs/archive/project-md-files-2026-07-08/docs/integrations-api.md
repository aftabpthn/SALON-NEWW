...
Define the outward-facing API surface: API keys, webhooks, versioning, rate limits and partner integration contracts.

## 2. Scope

- API key issuance, scoping and rotation
- Outbound webhooks with signing and retries
- Versioned public endpoints under /api/v1
- Rate limiting and quota per key

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- API keys are hashed at rest and scoped to tenant + permissions
- Webhook deliveries are signed, retried with backoff and visible in a delivery log
- Breaking changes only in a new version; v1 contracts stay stable
...
1. Partner creates key → scopes chosen → key shown once
2. Domain events → webhook fan-out per subscription → delivery tracked
3. Deprecations announced in CHANGELOG.md with sunset dates
...
- A leaked key can be rotated without downtime
- Webhook consumers can verify signatures with documented scheme
- Envelope responses follow API_GUIDELINES.md exactly
