...
Define transactional and marketing email: templates, provider adapter, deliverability, tracking and consent.

## 2. Scope

- Transactional email (invoices, confirmations, resets)
- Marketing campaigns and festival templates
- Provider adapter with API-key config per tenant
...
- Emails render from versioned templates in server/templates
- Bounces and complaints suppress future sends automatically
- Marketing email respects consent and unsubscribe links
...
1. Event → template rendered with tenant branding → queued
2. Worker sends via provider adapter → status webhook updates delivery state
3. Campaign sends batched and throttled
...
- No email sent to suppressed/unsubscribed addresses
- Template changes are previewable before activation
- Delivery status visible in the client timeline
