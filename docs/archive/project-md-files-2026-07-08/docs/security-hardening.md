...
Operational hardening checklist: headers, rate limiting, session management, encrypted secrets, dependency hygiene and the Enterprise Security Shield.

## 2. Scope

- Rate limiting and API protection headers
- Session management and refresh-token rotation
- Encrypted secrets and key management
- Dependency audit cadence
- Enterprise Security Shield controls (see docs/ENTERPRISE-SECURITY-SHIELD.md)

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Every new endpoint ships with auth, tenant scoping and rate limits by default
- Protected actions (void, delete, export) require elevated permission + audit log
- npm audit reviewed on a fixed cadence; criticals patched within SLA
...
1. Threat modelled change → controls chosen from this checklist → implemented + tested
2. security-shield.test.js and protected-actions.test.js guard regressions
3. Quarterly hardening review updates this document
...
- No endpoint is reachable without authentication except documented public ones
- Secrets never appear in code, logs or client bundles
- Root SECURITY.md policies are all implemented or tracked
