...
Define the advanced analytics engine: revenue forecasting, peak hours, staff productivity, churn risk, LTV, funnels and branch comparison.

## 2. Scope

- Revenue forecasting and peak-hour analysis
- Staff productivity scoring and repeat-customer analytics
- Churn risk, lifetime value and heatmaps
- Conversion funnel and branch comparison

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Analytics reads are isolated from OLTP hot paths (snapshot tables / scheduled jobs)
- Scores and forecasts store their model version and computed-at timestamp
- ml-service (Python: CLV, uplift) is called through a narrow adapter with timeouts
...
1. Nightly jobs aggregate facts into snapshot tables per tenant+branch
2. Dashboards read snapshots; drill-through goes to source rows
3. ML scores (CLV, uplift, churn) refreshed on schedule and cached
...
- Analytics queries never lock or slow POS billing
- Every derived metric is explainable: inputs + formula documented here
- Forecast accuracy is tracked against actuals
