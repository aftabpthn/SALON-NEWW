...
Define discount types, approval controls, happy hours, coupon codes and discount-intelligence reporting.

## 2. Scope

- Line and bill-level discounts (percent / flat)
- Discount approval thresholds and permissions
- Happy hours and time-based offers
- Coupons and campaign offers
- Sales discount intelligence report

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Discounts above threshold require approver role and are audit-logged
- Discount application order is deterministic and documented
- Every discount stores reason, actor and approver
...
1. Discount applied at POS → threshold checked → approval requested if needed
2. Happy-hour rules auto-apply within configured windows
3. Discount intelligence report surfaces outliers and leakage
...
- A discount can never push a line or invoice negative
- happy-hours-production-wiring.test.js and sales-discount-intelligence-report.test.js pass
- Unauthorized discount attempts are rejected and logged
