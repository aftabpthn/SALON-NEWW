...
Define the audit and compliance ledger: what is recorded, immutability rules, retention and how audits are queried.

## 2. Scope

- Audited events: booking creation, bill edits, client deletion, payment changes, discount approvals, logins, permission changes
- Actor, tenant, branch, timestamp and before/after payloads
...
- Audit writes happen in the same transaction as the audited change
- Audit rows are append-only; there is no update or delete path
- Sensitive payload fields are redacted per logging.md rules
...
1. Protected/mutating action → audit entry with actor + context
2. Super admin actions land in super_admin_audit
3. Compliance query → filtered, paginated, exportable audit view
...
- audit-log.test.js passes
- Every protected action produces exactly one audit row
- Audit retention meets the policy table in this document
