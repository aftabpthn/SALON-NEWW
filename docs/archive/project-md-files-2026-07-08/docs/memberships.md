...
Define membership plans, sales, redemptions at POS, renewal automation and revenue recognition.

## 2. Scope

- Membership plan definitions (discount, credit and service-count types)
- Sale, activation, freeze, transfer and expiry
- POS redemption and benefit application order
- Renewal reminders and membership revenue reporting

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Benefits apply deterministically (membership → package → loyalty → manual discount order)
- Membership liability is tracked as deferred revenue until redeemed
- Expiry and renewal jobs are idempotent
...
1. Plan sold at POS → payment captured → deferred revenue journal entry
2. Visit billed → eligible benefits auto-suggested → redemption recorded
3. Nearing expiry → workflow engine sends renewal reminders
...
- A benefit can never be redeemed beyond its remaining balance/count
- Membership state changes are audit-logged with actor and reason
- membership-*.test.js suites pass
