...
Define in-app, push and channel notifications: queues, device registration, delivery status and user preferences.

## 2. Scope

- In-app notification feed and realtime WebSocket delivery
- Mobile push notification queues and device registration
...
- Notifications are queued, not sent inline with request handling
- Each notification stores tenantId, recipient, channel, status and payload
- Preferences and quiet hours are respected at dispatch time
...
1. Domain event → notification record queued → worker dispatches per channel
2. WebSocket pushes to online sessions; push queue covers offline devices
3. Failures retried with backoff; permanent failures marked and visible
...
- No duplicate notification for the same event + recipient
- Notification dispatch failures never fail the originating business transaction
- Unread counts are consistent across devices
