...
Define staff master, attendance, shifts, commissions, incentives, payroll export and productivity intelligence.

## 2. Scope

- Staff master definitions and branch assignment
- Attendance (including biometric/camera capture) and shift planning
- Dynamic commission policies and incentive profiles
- Payroll export and payout accounting
- Productivity ranking and performance insights

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Commission calculation matches the assigned policy version at invoice time
- Attendance categories drive payroll inputs deterministically
- Staff see only their own data via staff-login self reports
...
1. Staff onboarded → role + branch assignment → services mapped
2. Invoice completed → commission line computed from policy → accrued for payout
3. Month end → payroll export with attendance, commissions, incentives and deductions
...
- Changing a commission policy never rewrites historical accruals
- Payout posting creates balanced journal entries in integer paise
- staff-*.test.js suites pass
