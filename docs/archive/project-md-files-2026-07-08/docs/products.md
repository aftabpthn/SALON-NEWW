...
Define the product master: retail vs consumable classification, pricing, barcodes, GST mapping and product sales reporting.

## 2. Scope

- Product master with category, brand, unit and barcode
- Retail vs in-house consumable classification
- Pricing, MRP, cost and GST rate mapping
- Product in/out and product-sales reports

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Every product carries a GST rate and HSN mapping used at billing time
- Cost updates flow into WMA valuation, never overwrite history
- Product deactivation hides from POS but preserves reporting history
...
1. Product created → category + tax + unit assigned → stock initialised per branch
2. Retail sale at POS → price + GST from master → stock deducted
3. Consumables consumed via service recipes (see inventory-recipes.md)
...
- Barcode/SKU is unique per tenant
- Price changes are versioned with effective dates
- product-*-report tests pass
