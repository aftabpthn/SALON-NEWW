...
- CI runs a critical audit gate through `npm run security:audit`; high audit report-only findings stay tracked until upstream non-breaking fixes are available.
- The Phase 5 production readiness gate checks secret scan, critical audit, focused security tests, CodeQL hardening, backup drill wiring, and WAF/CDN evidence wiring.
