...
> **Primary AI Role:** Backend Engineer
...
Define file/asset handling: uploads (logos, client photos, consent forms, attendance captures), storage layout, validation and serving.

## 2. Scope

- Upload validation (type, size, dimensions)
- Tenant-scoped storage layout on disk
- Serving with authorization checks
- Cleanup of orphaned files and backup inclusion

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Uploads are validated by content (magic bytes), not extension alone
- Files are stored outside the web root and served through authorized endpoints
- Every stored file maps to a database row with tenantId + owner entity
...
1. Upload → validate → store under tenant path with generated name → row persisted
2. Serve → auth + tenant check → stream with correct content type
3. Scheduled job removes orphans and includes files in backups
...
- No path traversal: generated names only, no user-controlled paths
- A tenant can never fetch another tenant’s file by guessing URLs
- Consent forms and biometric captures follow retention policy
