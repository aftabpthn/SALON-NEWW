...
Define SMS messaging: DLT-compliant templates (India), provider adapter, delivery reports and cost tracking.

## 2. Scope

- DLT template registration and mapping (India compliance)
- Transactional SMS (OTP, confirmations, reminders)
- Promotional SMS with consent gating
- Delivery reports and per-tenant cost metering

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Only registered DLT templates are sent for Indian numbers
- OTP messages are highest priority and never queued behind campaigns
- Per-message cost recorded for SaaS metering
...
1. Message request → template + variables validated → provider adapter send
2. Delivery report webhook → status updated in message log
3. Monthly usage rolled into saas billing metering
...
- Template variable mismatches are rejected before send
- Promotional SMS blocked for opted-out clients
- Costs reconcile with provider invoice
