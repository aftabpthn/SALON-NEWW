...
Define logging standards: structured logs, levels, correlation ids, PII redaction and retention.

## 2. Scope

- Structured JSON logs with level, timestamp, tenantId, requestId
- Request/response logging with sensitive-field redaction
- Worker and scheduler job logs
...
- Logs never contain passwords, tokens, OTPs, card data or raw PII
- Every request carries a correlation id propagated to jobs it spawns
- Log level is environment-configurable without code change
...
1. Request in → requestId assigned → logged at boundaries only (no chatty logs in loops)
2. Errors logged once, at the handling layer, with stack + context
3. Rotation keeps disk usage bounded
...
- Grep by requestId reconstructs a full request path
- Redaction list in this doc is enforced by the logger utility
- Error logs alone are sufficient to reproduce reported bugs
