...
Define schema migrations (server/migrations) and customer data imports (from Excel/other CRMs) with validation and rollback.

## 2. Scope

- Schema migrations: ordering, idempotency and rollback notes
- Customer data import from spreadsheets and competitor exports
- Data Migration Center workflows (see docs/DATA_MIGRATION_CENTER.md)
- Dry-run validation and error reporting

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Migrations are additive-first; destructive changes need explicit approval (Delete Safety Rule)
- Imports are dry-run validated with a row-level error report before commit
- Every imported row is tagged with import batch id for traceability/undo
...
1. New migration file → sequential id → applied on boot exactly once
2. Import: upload → map columns → dry-run report → commit in transaction batches
3. Failed batches roll back cleanly; partial imports are impossible
...
- migration.test.js passes
- Re-running a migration or import batch is a no-op
- Imports enforce tenantId + branchId on every row
