...
Define the client master: profiles, visit history, wallet, loyalty, notes, tags, consent, WhatsApp history and the Customer 360 intelligence view.

## 2. Scope

- Client profiles, master definitions and dedupe rules
- Visits, purchase history, membership and wallet
- Notes timeline, tags, important dates, consent forms
- Customer 360: LTV, risk score, favourite service, preferred staff

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- One canonical client record per person per tenant (phone-number keyed dedupe)
- Consent flags gate every outbound channel
- Client deletion is a protected action recorded in the audit ledger
...
1. New client via POS / booking / import → normalized (auto name-case) → deduped
2. Every interaction (visit, invoice, message) appends to the 360 timeline
3. Churn-risk and next-best-action snapshots refreshed by scheduled jobs
...
- Search by phone/name returns in under 200 ms with 100k clients
- Wallet balance derives from ledger rows, never a mutable counter alone
- PII exposure limited by role — see RBAC.md
