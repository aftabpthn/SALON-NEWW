...
Define supplier master, purchase orders, purchase bills (including drafts), GRN, supplier payments and purchase analytics.

## 2. Scope

- Supplier master with GST details and payment terms
- Purchase orders and purchase bill drafts
- Goods receipt with batch + expiry capture
- Supplier payments, dues and ageing

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Purchase bills post stock and accounting atomically in one transaction
- Supplier dues reconcile to accounts-payable ledger
- Draft bills never affect stock or accounting until confirmed
...
1. Low-stock / AI reorder suggestion → PO raised → sent to supplier
2. Goods received → batches recorded → bill confirmed → stock + AP posted
3. Payment made → due reduced → journal entry posted
...
- Confirming the same bill twice is a no-op (idempotent)
- purchase-bill-drafts.test.js passes
- Supplier ageing report matches AP ledger to the paisa
