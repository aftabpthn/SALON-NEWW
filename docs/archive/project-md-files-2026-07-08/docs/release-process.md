...
> **Primary AI Role:** Release Manager
...
- **done**: Core release workflow, gating rules, changelog/rollback obligations, and architecture/security constraints are defined.
- **in-progress**: Release owner handoff templates and commandable verification checklists are being normalized across docs.
- **blocker**: None.

## 1. Purpose

Define how a release goes out: versioning, quality gates, changelog, migration review, deploy, verification and rollback decision points.

## 2. Scope

- Semantic-ish versioning of the app (package.json version)
- Quality gate: npm run quality (check:server + tests + build)
- CHANGELOG.md update rules
- Canonical changelog cadence and release-note rhythm
- Migration review for destructive changes
- Deploy + post-deploy verification + rollback criteria

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- No release without a green quality gate and updated CHANGELOG.md
- Destructive migrations need explicit sign-off and a rehearsed rollback
- Every release is taggable and traceable to commits
...
1. Freeze → npm run quality → CHANGELOG.md updated → version bumped
2. Backup taken → deploy per docs/deployment.md → smoke checks
3. Watch monitoring for 30 minutes → rollback if criteria met → announce

### 6.1 Release Log Cadence

- **Per release (mandatory):**
  - Update `CHANGELOG.md` with version, date, breaking/non-breaking notes, migration notes, and rollback plan.
  - Add the release owner and impacted area tags (`[api]`, `[ui]`, `[ops]`, `[security]`).
  - Link the release to relevant audit/sign-off docs (`PROJECT_AUDIT_REPORT.md`, migration signoff files, QA notes).
- **Cadence (canonical):**
  - **Every release:** canonical changelog entry in `CHANGELOG.md`.
  - **Weekly:** capture operationally relevant changes in the release notes section and verify no open high-risk items remain unlogged.
  - **Monthly:** review `CHANGELOG.md` for missed release entries and archive a period summary under `docs/` if needed for audit traceability.
...
- A release can be rolled back within RTO using the documented steps
- CHANGELOG.md entry exists for every released version
- Failed releases produce a post-mortem entry
