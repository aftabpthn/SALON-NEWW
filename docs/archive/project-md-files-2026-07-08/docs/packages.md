...
Define prepaid service packages: definitions, sale, session tracking, redemption, expiry and refund rules.

## 2. Scope

- Package definitions (services, session counts, validity)
- Package sale and payment
- Session redemption at POS with balance tracking
- Expiry, extension and refund policy

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Remaining sessions are derived from redemption rows, never a free-standing counter
- Package sale creates deferred revenue; each redemption recognises the proportional amount
- Cross-branch redemption follows tenant policy flags
...
1. Package sold → sessions ledger initialised → deferred revenue posted
2. Service billed → open package auto-suggested → session consumed
3. Expiry job flags lapsed packages → optional win-back workflow
...
- Session balance can never go negative
- Package definitions are first-class CRUD resources (not static placeholders)
- Refunds reverse unredeemed value only, with balanced journal entries
