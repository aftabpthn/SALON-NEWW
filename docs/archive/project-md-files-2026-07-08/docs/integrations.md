...
Catalogue every external system AuraShine talks to (WhatsApp, payment gateways, email/SMS providers, marketplace connectors) and the standards all integrations follow.
...
- Keep all provider credentials tenant-scoped and encrypted at rest
- Verify webhook signatures before processing any inbound payload
- Isolate provider failures so a down integration never blocks POS billing
...
1. New integration → adapter service in server/services with a narrow interface
2. Secrets loaded from environment / encrypted settings, never hard-coded
3. Outbound calls wrapped with timeout + retry + failure logging
...
- No API key or secret appears in source, logs or error messages
- Every webhook endpoint validates signature and tenant mapping before writes
- Provider outage degrades gracefully with queued retries
