...
Operational guide for tenancy: onboarding, domain mapping, subscriptions, usage metering, suspension and tenant health.

## 2. Scope

- SaaS onboarding (tenant + trial + owner + first branch in one workflow)
- Domain mapping and verification
- Subscription states, plan limits and usage events
- Suspension controls and tenant health via super admin console

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Every tenant-owned table has tenantId; every read/write is tenant-scoped in repositories
- Tenant context resolves from x-tenant-id header or verified domain mapping
- Plan limits are enforced at write time with friendly errors
...
1. Onboarding API → tenant, subscription, owner, branch created atomically
2. Requests resolve tenant → middleware injects context → repositories enforce scope
3. Usage events roll up for metering and super admin analytics
...
- tenant-safety.test.js and billing-tenant-isolation.test.js pass
- A tenant can never read or write another tenant’s rows — verified by tests
- See root TENANT_ARCHITECTURE.md for design rationale
