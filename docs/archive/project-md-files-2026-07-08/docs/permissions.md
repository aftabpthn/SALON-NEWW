...
The authoritative permission matrix: which role can perform which action on which resource, including custom roles and protected actions.

## 2. Scope

- Built-in roles: owner, manager, receptionist, staff, accountant, inventory manager, analyst, superAdmin
- Custom role definitions
- Protected actions (void invoice, delete client, discount approval, exports)
...
- Every route declares its required permission; deny by default
- UI hides what the API forbids, but the API is the enforcement point
- Custom roles compose from the same named permissions, never bypass them
...
1. New feature → permission named here first → middleware guard → UI gating
2. Role changes take effect on next request (no re-login required, tokens re-validated)
3. Matrix reviewed with every release
...
- No API mutation exists without a permission mapping in this matrix
- See root RBAC.md for architecture; this file holds the live matrix
