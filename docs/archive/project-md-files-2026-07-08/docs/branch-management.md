...
Define branches (locations) within a tenant: creation, staff assignment, branch-scoped data, cross-branch roles and branch branding.

## 2. Scope

- Branch creation and configuration
- Branch-scoped stock, appointments, billing and reports
- Cross-branch access rules by role
- Branch-specific branding and settings

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Every business table carries branchId alongside tenantId
- Owner/admin/manager/analyst work across branches; staff/front-desk are limited to assigned branch ids
- Branch comparison analytics never leak across tenants
...
1. Branch created → default settings + chairs/resources configured
2. Users assigned branch ids → x-branch-id header scopes each request
3. Branch dashboards and comparisons read branch-scoped snapshots
...
- A staff user cannot query another branch even with a forged header (server-side check)
- Branch deletion is protected — requires empty/migrated data and explicit approval
- Branch filters exist on every list API and report
