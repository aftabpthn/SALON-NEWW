...
Define what is monitored and alerted on: API health, error rates, latency, queue depth, disk, backups and business KPIs.

## 2. Scope

- Health endpoints (/api/health) and uptime checks
- Error-rate and latency monitoring per route group
- Worker/queue depth and job failures
- Disk usage (SQLite + backups) and KPI monitors

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Every alert has a documented owner and runbook link
- Alert thresholds distinguish page-worthy from ticket-worthy
- Tenant-level anomalies (billing stopped, message failures) surface to super admin
...
1. Metrics collected from API + workers → dashboards per service
2. Threshold breach → alert with runbook link → on-call responds
3. Weekly review tunes noisy alerts
...
- A dead API, stuck worker or failed backup alerts within 5 minutes
- No alert fires without an actionable runbook
- KPI monitor rules are tenant-scoped
