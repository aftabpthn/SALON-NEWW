...
Define the Razorpay gateway integration: order creation, checkout, webhook verification, reconciliation, refunds and settlement matching.

## 2. Scope

- Order + payment flow for online booking deposits and invoice payments
- Webhook signature verification and idempotent processing
- Refund initiation and status tracking
- Settlement reconciliation against payouts

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Webhooks are verified (signature) and deduplicated (event id) before any write
- Gateway amounts are integer paise natively — never convert through floats
- Payment capture updates invoices only through the standard payments service
...
1. Order created server-side with amount in paise → checkout on client
2. payment.captured webhook → verify → idempotency check → payment row + journal entry
3. Daily settlement file matched to captured payments; mismatches flagged
...
- Replayed webhooks never double-credit an invoice
- Secrets live in env config; test/live mode is explicit per tenant
- billing-webhook-security.test.js passes
