...
> **Primary AI Role:** Inventory Domain Expert
...
Define how stock is tracked, deducted, valued and audited across every branch: products, batches, expiry, reorder intelligence and waste analysis.

## 2. Scope

- Product stock levels per branch
- Batch tracking and expiry alerts
- Auto deduction on POS sale and service consumption
- AI reorder suggestions and purchase prediction
- Waste/shrinkage analysis and stock audits

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Keep stock quantities correct after every billing, transfer, purchase and adjustment event
- Guarantee tenant + branch isolation on every inventory table
- Value inventory with WMA (weighted moving average) costing consistent with the accounting engine
...
1. Purchase bill received → stock in with batch + cost → WMA recalculated
2. POS sale / service performed → auto deduction (batch-aware, FEFO for expiring items)
3. Periodic stock audit → variance report → approved adjustment entry
...
- Stock never goes negative without an explicit override flag and audit log entry
- Every stock movement row carries tenantId, branchId, reason and reference id
- Deduction is idempotent per invoice — re-processing an invoice never double-deducts
