...
Define the payment model: methods, split payments, advances/deposits, refunds, partial payments, dues and the single source of payment truth.

## 2. Scope

- Payment methods: cash, card, UPI, wallet, gateway
- Split and partial payments against invoices
- Advances/deposits and their settlement
- Refunds and due recovery

Out of scope for this document: anything covered by another domain doc — link to it instead of duplicating.

## 3. Responsibilities

- Payment rows are immutable; corrections are reversing entries
- Invoice paid-status derives from payment rows (payment truth), never a flag set by hand
- Every payment posts balanced journal entries in integer paise
...
1. Payment captured at POS → row written in the same transaction as invoice state change
2. Advance taken at booking → held as liability → auto-suggested at billing
3. Refund → permission-gated → reversing payment + journal entry
...
- pos-invoice-payment-truth.test.js and payment-collection.test.js pass
- Sum of payments never exceeds invoice total
- Dues list reconciles with unpaid-due-recovery report
