  ...
  SELECT EXISTS (
    SELECT 1
      ...
      JOIN branches target_branch
       ...
       AND target_branch.id::TEXT = p_target_branch_id
     ...
     WHERE source_branch.tenant_id::TEXT = p_tenant_id
       AND source_branch.id::TEXT = p_source_branch_id
       AND source_branch.active = TRUE
       AND p_source_branch_id <> p_target_branch_id
       AND source_client.normalized_phone <> ''
