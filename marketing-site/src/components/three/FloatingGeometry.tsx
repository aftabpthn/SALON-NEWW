...
export function FloatingGeometry({ variant = "hero" }: FloatingGeometryProps) {
  ...
  if (!capabilities.ready || !capabilities.webgl || capabilities.coarsePointer || capabilities.viewport === "compact" || capabilities.reducedMotion) {
    return <div className="absolute inset-0 z-0 grid place-items-center opacity-40" aria-hidden="true"><div className="h-40 w-40 rounded-[2.5rem] bg-aura-burgundy/15 shadow-[0_0_80px_rgba(47,120,149,.16)]" /></div>;
  }
  if (variant === "minimal") {
    return (
      <div className="absolute inset-0 z-0 pointer-events-none">
        <Canvas camera={{ position: [0, 0, 6], fov: 50 }} dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }} style={{ background: "transparent" }}>
          ...
          <Orb position={[0, 0, 0]} color="#123f5c" size={1} speed={1.5} distort={0.3} />
        </Canvas>
      </div>
    );
  }
  ...
  return (
    <div className="absolute inset-0 z-0 pointer-events-none">
      <Canvas camera={{ position: [0, 0, 8], fov: 55 }} dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }} style={{ background: "transparent" }}>
        ...
        <pointLight position={[-5, -5, -3]} intensity={0.4} color="#5ba9bd" />

        <Orb position={[-3, 1, -2]} color="#123f5c" size={1.3} speed={1.5} distort={0.4} />
        <Orb position={[3, -1, -3]} color="#5ba9bd" size={0.8} speed={2} distort={0.3} />
        <Orb position={[0, 2.5, -4]} color="#397f99" size={0.6} speed={1.2} distort={0.2} />
      </Canvas>
    </div>
  );
}
