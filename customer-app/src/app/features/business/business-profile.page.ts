              ...
              <div class="staff-grid">
                @for (staff of business().staff; track staff.id) {
                  <article class="staff-card premium-card">
                    ...
                    <ion-button size="small" fill="outline" class="secondary-button" [routerLink]="['/business', business().slug, 'book']" [queryParams]="{ staffId: staff.id }">Book with {{ staff.name.split(' ')[0] }}</ion-button>
                  </article>
                } @empty {
