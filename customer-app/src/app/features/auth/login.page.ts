        ...
        <section class="auth-card">
          ...
          @if (step === "choices") {
            ...
            <div class="social-stack">
              ...
              @if (showLocalDevLogin) {
                <ion-button expand="block" fill="outline" class="choice-button" (click)="continueWithLocalDevSession()" [disabled]="auth.loading()">
                  {{ localDevOpen ? "Start local dev session" : "Local dev session" }}
                </ion-button>
              }
            ...
            @if (showLocalDevLogin && localDevOpen) {
              <form class="auth-form local-dev-form" (submit)="continueWithLocalDevSession($event)">
                <ion-list>
                  <ion-item lines="none">
                    <ion-input label="Dev email" labelPlacement="stacked" type="email" inputmode="email" autocomplete="off" [(ngModel)]="localDevEmail" name="localDevEmail"></ion-input>
                  </ion-item>
                  <ion-item lines="none">
                    <ion-input label="First name" labelPlacement="stacked" type="text" autocomplete="off" [(ngModel)]="localDevFirstName" name="localDevFirstName"></ion-input>
                  </ion-item>
                  <ion-item lines="none">
                    <ion-input label="Last name" labelPlacement="stacked" type="text" autocomplete="off" [(ngModel)]="localDevLastName" name="localDevLastName"></ion-input>
                  </ion-item>
                  <ion-item lines="none">
                    <ion-input label="Phone" labelPlacement="stacked" type="tel" inputmode="tel" autocomplete="off" [(ngModel)]="localDevPhone" name="localDevPhone"></ion-input>
                  </ion-item>
                  <ion-item lines="none">
                    <ion-input label="Dev session secret" labelPlacement="stacked" type="password" autocomplete="off" [(ngModel)]="localDevSecret" name="localDevSecret"></ion-input>
                  </ion-item>
                </ion-list>
              </form>
            }
...
export class LoginPage implements OnInit, OnDestroy {
  ...
  readonly showLocalDevLogin = !environment.production && typeof window !== "undefined" && ["localhost", "127.0.0.1"].includes(window.location.hostname);
  ...
  constructor(readonly auth: AuthService, private readonly router: Router, private readonly route: ActivatedRoute, private readonly api: CustomerApiService) {
  ...
  async continueWithLocalDevSession(event?: Event) {
    event?.preventDefault();
    if (!this.showLocalDevLogin) return;
    if (!this.localDevOpen) {
      this.localDevOpen = true;
      this.notice = "";
      return;
    }
    const slug = this.bookingSlugFromReturnUrl();
    if (!slug) {
      this.notice = "Open local dev session from a booking page return URL.";
      return;
    }
    const email = this.localDevEmail.trim().toLowerCase();
    const phone = this.localDevPhone.trim();
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      this.notice = "Enter a valid customer email.";
      return;
    }
    if (!this.isValidFullPhone(phone)) {
      this.notice = "Enter a valid customer phone with country code.";
      return;
    }
    const firstName = this.localDevFirstName.trim();
    const lastName = this.localDevLastName.trim();
    const secret = this.localDevSecret.trim();
    if (!secret) {
      this.notice = "Local dev session secret is required.";
      return;
    }
    this.notice = "";
    try {
      const business = await firstValueFrom(this.api.getPublicBusiness(slug));
      await this.auth.issueDevSession({
      ...
      this.notice = error instanceof Error ? error.message : "Unable to create local customer session.";
    }
  }
  ...
  private bookingSlugFromReturnUrl(): string {
    const target = this.safeReturnUrl(this.route.snapshot.queryParamMap.get("returnUrl"));
    const match = /^\/business\/([^/?#]+)\/book(?:[/?#]|$)/.exec(target);
