        ...
        <section class="hero">
          ...
          <aside class="live-consultation-card" aria-label="Live AI consultation">
            ...
            <label class="consultation-input-label">
              Language
              <select [ngModel]="consultationLanguage()" (ngModelChange)="setConsultationLanguage($event)">
            ...
            @if (consultationResponse(); as response) {
              <div class="consultation-results">
                ...
                @if (response.bookingConfirmationPolicy) {
                  <p class="consultation-safety">{{ response.bookingConfirmationPolicy }}</p>
                }
              </div>
            }
          </aside>
...
export class HomePage implements OnInit {
  ...
  readonly consultationLanguage = signal(localStorage.getItem("aura_customer_language") || "en-IN");
  consultationText = "";
  ...
  setConsultationLanguage(language: string) {
    const value = language === "hi-IN" ? "hi-IN" : "en-IN";
    localStorage.setItem("aura_customer_language", value);
    this.consultationLanguage.set(value);
  }
  ...
  async sendConsultation() {
    ...
    try {
      const response = await firstValueFrom(this.api.createLiveConsultation({
        ...
        languageCode: this.consultationLanguage()
      }));
