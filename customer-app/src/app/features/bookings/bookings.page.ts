        ...
        <section class="bookings-hero">
          ...
          <div class="booking-command-grid">
            @for (item of bookingCommands; track item.label) {
              <button type="button" class="command-card premium-card" [disabled]="item.action === 'quickbook' && !quickBookCandidate()" (click)="runBookingCommand(item.action)">
            ...
            @for (booking of filtered(); track booking.id) {
              ...
              >
                ...
                <div class="booking-content">
                  ...
                  <div class="actions">
                    @if (booking.status === "waitlist_offer") {
                      <ion-button size="small" class="primary-gradient" [disabled]="!!actionLoading()" (click)="acceptWaitlistOffer($event, booking.id)">Accept slot</ion-button>
                      <ion-button size="small" fill="outline" color="medium" [disabled]="!!actionLoading()" (click)="declineWaitlistOffer($event, booking.id)">Decline</ion-button>
                    }
          ...
          <section class="waitlist-sheet" role="dialog" aria-modal="true" aria-label="Join appointment waitlist" (click)="$event.stopPropagation()">
            ...
            <div class="waitlist-body">
              ...
              <div class="waitlist-date-range">
                ...
                <label class="waitlist-field">
                  <span>To date</span>
                  <input type="date" [min]="dialog.preferredDate || todayKey()" [(ngModel)]="dialog.preferredEndDate" name="waitlistEndDate" (ngModelChange)="updateWaitlist({ preferredEndDate: $event })" />
                </label>
              </div>
...
export class BookingsPage implements OnDestroy, OnInit {
  ...
  readonly filtered = computed(() => this.marketplace.bookings().filter((booking) => this.bookingMatchesTab(booking)));
  ...
  readonly bookingCommands = [
    { label: "QuickBook", copy: "Repeat past visits faster", icon: "repeat-outline", action: "quickbook" },
    { label: "Waitlist", copy: "Join auto-fill queues", icon: "hourglass-outline", action: "waitlist" },
    { label: "Digital check-in", copy: "Arrival and consent ready", icon: "checkmark-circle-outline", action: "checkin" },
    { label: "Support", copy: "Chat and ticket handoff", icon: "chatbubbles-outline", action: "support" }
  ];
  ...
  runBookingCommand(action: string) {
    if (action === "quickbook") {
      const booking = this.quickBookCandidate();
      if (booking) this.rebookFromBooking(booking);
      return;
    }
    if (action === "waitlist") {
    ...
    if (action === "checkin") {
      const booking = this.marketplace.bookings().find((item) => this.canManageUpcoming(item));
      if (booking) void this.router.navigate(["/bookings", booking.id]);
      else this.presentToast("No upcoming booking available for check-in.", "warning");
      return;
    }
    void this.router.navigateByUrl("/tabs/support");
  }

  quickBookCandidate(): Booking | null {
    return this.marketplace.bookings()
      .filter((booking) => this.canRebook(booking))
      .sort((a, b) => this.bookingTime(b) - this.bookingTime(a))[0] ?? null;
  }
  ...
  rebook(event: Event, booking: Booking) {
    event.preventDefault();
    event.stopPropagation();
    this.rebookFromBooking(booking);
  }

  private rebookFromBooking(booking: Booking) {
    if (booking.businessId) {
      void this.router.navigate(["/business", booking.businessId, "book"], {
        queryParams: {
          serviceId: booking.serviceId || undefined,
          serviceIds: booking.serviceIds?.length ? booking.serviceIds.join(",") : undefined,
  ...
  canManageUpcoming(booking: Booking): boolean {
    return this.tab() === "upcoming" && !["cancelled", "completed", "waitlist_offer"].includes(booking.status);
  }

  private bookingMatchesTab(booking: Booking): boolean {
    if (booking.status === "waitlist_offer") return this.tab() === "upcoming";
    if (this.tab() === "past") return this.canRebook(booking) || this.bookingTime(booking) < Date.now();
    return !this.canRebook(booking) && this.bookingTime(booking) >= Date.now();
  }

  private bookingTime(booking: Booking): number {
    const source = booking.startAt || booking.startsAt || "";
    const time = source ? new Date(source).getTime() : 0;
    return Number.isFinite(time) ? time : 0;
  }

  async acceptWaitlistOffer(event: Event, id: string) {
    event.preventDefault();
    event.stopPropagation();
    this.actionLoading.set(`waitlist-offer:${id}`);
    try {
      await this.marketplace.acceptWaitlistOffer(id);
      await this.presentToast("Your waitlist slot is confirmed.", "success");
      this.reload();
    } catch {
      await this.presentToast(this.marketplace.error() || "Unable to accept this waitlist slot.", "danger");
    } finally {
      this.actionLoading.set("");
    }
  }

  async declineWaitlistOffer(event: Event, id: string) {
    event.preventDefault();
    event.stopPropagation();
    this.actionLoading.set(`waitlist-offer:${id}`);
    try {
      await this.marketplace.declineWaitlistOffer(id);
      await this.presentToast("Waitlist slot declined.", "success");
      this.reload();
    } catch {
      await this.presentToast(this.marketplace.error() || "Unable to decline this waitlist slot.", "danger");
    } finally {
      this.actionLoading.set("");
    }
  }
  ...
  async joinWaitlist(event: Event) {
    ...
    this.waitlistDialog.set({
      ...
      preferredEndDate: this.dateValue(booking),
      preferredTime: "any",
      priority: "normal",
      reason: "",
      error: ""
  ...
  async submitWaitlist() {
    ...
    if (dialog.preferredEndDate && !/^\d{4}-\d{2}-\d{2}$/.test(dialog.preferredEndDate)) {
      this.updateWaitlist({ error: "Choose a valid end date." });
      return;
    }
    if (dialog.preferredEndDate && dialog.preferredEndDate < dialog.preferredDate) {
      this.updateWaitlist({ error: "End date cannot be before start date." });
      return;
    }
    await this.joinWaitlistForBooking(dialog.booking, {
      preferredDate: dialog.preferredDate,
      preferredEndDate: dialog.preferredEndDate,
  ...
  private async rescheduleToSlot(booking: Booking, startAt: string, slot?: AvailabilitySlot) {
    this.actionLoading.set(`reschedule:${booking.id}`);
    try {
      const result = await this.marketplace.rescheduleBooking(booking.id, { startAt, staffId: slot?.staffId || booking.staffId || undefined });
      await this.presentToast(result.pendingApproval ? "Reschedule request sent for manager approval." : "Booking rescheduled successfully.", "success");
  ...
  private async joinWaitlistForBooking(booking: Booking, value: { preferredDate?: string; preferredEndDate?: string; preferredTime?: WaitlistDialog["preferredTime"]; priority?: "normal" | "high"; reason?: string }) {
    ...
    try {
      ...
      const rangeNote = value.preferredEndDate && value.preferredEndDate !== value.preferredDate ? `Preferred date range: ${value.preferredDate} to ${value.preferredEndDate}.` : "";
      ...
      const result = await this.marketplace.joinBookingWaitlist(booking.id, {
        ...
        preferredStartDate: value.preferredDate || this.dateValue(booking),
        preferredEndDate: value.preferredEndDate || value.preferredDate || this.dateValue(booking),
        reason: [rangeNote, timeNote, customerNote || "Customer wants an earlier or backup slot"].filter(Boolean).join(" "),
