...
import { AuthSession, CustomerDeviceInfo, CustomerDeviceSession, CustomerDevSessionPayload, CustomerNotificationPreferences, CustomerProfile, OtpRequestResponse } from "./api.types";
...
export class AuthService {
  ...
  async issueDevSession(payload: Omit<CustomerDevSessionPayload, "device">, secret: string): Promise<AuthSession> {
    this.loading.set(true);
    this.error.set("");
    try {
      const response = await firstValueFrom(this.api.issueDevSession({ ...payload, device: this.deviceInfo() }, secret.trim()));
      this.saveSession(response);
      const profile = await this.loadMe();
      return { ...response, customer: profile };
    } catch (error) {
      this.error.set(this.message(error, "Unable to create local customer session"));
      throw error;
    } finally {
      this.loading.set(false);
    }
  ...
  async loadMe(): Promise<CustomerProfile> {
    ...
    this.error.set("");
    try {
      const profile = this.normalizeProfile(await firstValueFrom(this.api.getMe()));
  ...
  async updateMe(payload: Partial<CustomerProfile>): Promise<CustomerProfile> {
    ...
    this.error.set("");
    try {
      const profile = this.normalizeProfile(await firstValueFrom(this.api.updateMe(this.profileUpdatePayload(payload))));
  ...
  async verifyProfileEmailCode(email: string, code: string): Promise<CustomerProfile> {
    ...
    this.error.set("");
    try {
      const profile = this.normalizeProfile(await firstValueFrom(this.api.verifyProfileEmailCode(email.trim(), cleanCode)));
      this.customer.set({ ...profile, isLoggedIn: true });
      return this.customer() as CustomerProfile;
    } catch (error) {
      this.error.set(this.message(error, "Unable to verify email code"));
  ...
  async verifyProfilePhoneOtp(phone: string, otp: string): Promise<CustomerProfile> {
    ...
    this.error.set("");
    try {
      const profile = this.normalizeProfile(await firstValueFrom(this.api.verifyProfilePhoneOtp(phone.trim(), cleanOtp)));
      this.customer.set({ ...profile, isLoggedIn: true });
      return this.customer() as CustomerProfile;
    } catch (error) {
  ...
  async logout(): Promise<void> {
    ...
    this.error.set("");
    try {
      await firstValueFrom(this.api.logout(this.getRefreshToken() || ""));
  ...
  async refreshSession(): Promise<AuthSession> {
    ...
    const customer = this.normalizeProfile(session.customer);
    const normalized = { ...session, customer };
  ...
  private saveSession(session: AuthSession) {
    ...
    this.customer.set({ ...this.normalizeProfile(session.customer), isLoggedIn: true });
    this.biometricLocked.set(false);
  }
  ...
  private profileUpdatePayload(payload: Partial<CustomerProfile>): Partial<CustomerProfile> {
    const firstName = String(payload.firstName || "").trim();
    const lastName = String(payload.lastName || "").trim();
    const current = (this.customer()?.notificationPreferences || {}) as Partial<CustomerNotificationPreferences>;
    const requested = (payload.notificationPreferences || {}) as Partial<CustomerNotificationPreferences>;
    const communicationPreferences = {
      preferredChannel: "none",
      whatsappOptIn: requested.promotions ?? current.promotions ?? false,
      smsOptIn: requested.bookingReminders ?? current.bookingReminders ?? false,
      emailOptIn: requested.membership ?? current.membership ?? false,
      bookingReminders: requested.bookingReminders ?? current.bookingReminders ?? false,
      promotions: requested.promotions ?? current.promotions ?? false,
      loyalty: requested.loyalty ?? current.loyalty ?? false,
      membership: requested.membership ?? current.membership ?? false
    };
    return {
  ...
  private normalizeProfile(profile: CustomerProfile): CustomerProfile {
    const firstName = String(profile.firstName || "").trim();
    const lastName = String(profile.lastName || "").trim();
    const communication = profile.communicationPreferences && typeof profile.communicationPreferences === "object"
      ? profile.communicationPreferences as Record<string, unknown>
      : {};
    const notificationPreferences = {
      bookingReminders: Boolean(communication["bookingReminders"] ?? communication["smsOptIn"]),
      promotions: Boolean(communication["promotions"] ?? communication["whatsappOptIn"]),
      loyalty: Boolean(communication["loyalty"] ?? communication["emailOptIn"]),
      membership: Boolean(communication["membership"] ?? communication["emailOptIn"])
    };
    return {
      ...profile,
      firstName,
      lastName,
      name: String(profile.name || profile.displayName || [firstName, lastName].filter(Boolean).join(" ") || profile.phone || "Customer").trim(),
      notificationPreferences,
      profileComplete: Boolean(profile.profileComplete) || Boolean(firstName && lastName && String(profile.email || "").trim() && String(profile.phone || "").trim())
    };
  }
}
